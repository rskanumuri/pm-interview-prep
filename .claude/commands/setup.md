...
- **Passive-pipeline default:** when a company surfaces through a passive/inbound pipeline (employed-and-exploring), role-key its files from the very first role. Companies recur over months in passive mode, so company-only keys force later migrations. Company-only keys are legacy, used only by pre-existing single-role pipelines.
- `role_slug` = role string lowercased, non-alphanumeric → underscores (e.g., "Senior PM, Data Governance" → `senior_pm_data_governance`). When the literal slug is unwieldy, a shortened sane slug is allowed and then standardized on (e.g., "Lead Product Manager, Data Platform Infrastructure" → `lead_pm_data_platform`).
